...
public:
	...
	LL find(const MyString& str, size_t index = 0);		//查找字符串str第一次出现的位置
	LL find(const char* str, size_t index = 0);			//同上,但是类型是char*
	LL findRK(const char* str, size_t index = 0);		//RK算法
	LL findRK(const MyString& str, size_t index = 0);	//RK算法
...
LL MyString::find(const MyString& str, size_t index) {
...
LL MyString::find(const char* str, size_t index) {
	int size = strlen(str);
...
LL MyString::findRK(const MyString& str, size_t index){
	return size_t();
}
